...
pub mod claim {
    ...
    pub fn claim(ctx: Context<Claim>) -> Result<()> {
        // Anchor 0.29: get bump from ctx.bumps
        let escrow_auth_bump = *ctx
            .bumps
            .get("escrow_auth")
            .ok_or(ErrorCode::MissingBump)?;
        let _seeds: &[&[u8]] = &[b"escrow_auth", &[escrow_auth_bump]];
...
pub struct InitPool<'info> {
    ...
    /// CHECK: seeds+bump enforce correctness
...
pub struct Claim<'info> {
    ...
    #[account(mut, seeds = [b"pool", escrow_auth.key().as_ref()], bump)]
    pub pool: Account<'info, Pool>,
